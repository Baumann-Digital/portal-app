<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://www.w3.org/1998/Math/MathML" exclude-result-prefixes="tei xs m" version="2.0" xpath-default-namespace="http://www.w3.org/1999/xhtml">
    ...
    <xsl:template match="img|video[@src]|audio[@src]|embed|source">
        ...
        <xsl:choose>
            <xsl:when test="parent::body or parent::div or parent::article         or parent::audio or parent::video">
                <p>
    ...
    <xsl:template match="@align|table/@border|table/@cellpadding|table/@cellspacing|table/@width">
    ...
    <xsl:template match="img[starts-with(@src,'data:')]">
        <xsl:message>UNHANDLED data: in <xsl:value-of select="name()"/>
        </xsl:message>
    </xsl:template>
    <xsl:template match="*">
        <xsl:message>UNTRANSLATED TAG <xsl:value-of select="name()"/>
        </xsl:message>
    ...
    <xsl:template match="*" mode="addnamespace">
        <xsl:element xmlns="http://www.w3.org/1999/xhtml" name="{name()}">
            <xsl:apply-templates select="@*|*|processing-instruction()|comment()|text()" mode="addnamespace"/>
        </xsl:element>
    </xsl:template>
</xsl:stylesheet>